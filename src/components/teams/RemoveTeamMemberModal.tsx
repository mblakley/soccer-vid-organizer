...
import { AdminDisplayTeamMember } from '@/lib/types/admin';

interface RemoveTeamMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  member: AdminDisplayTeamMember | null;
  isDarkMode: boolean;
  isRemoving: boolean;
}
          ...
          <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="sm:flex sm:items-start">
              <div className={`mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10 ${
                ...
                <svg
                  className={`h-6 w-6 ${isDarkMode ? 'text-red-200' : 'text-red-600'}`}
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                    Are you sure you want to remove {member.user_name} from the team? This action cannot be undone.
                  </p>
                </div>
              </div>
            </div>
