  ...
  BEGIN
    IF EXISTS (
      ...
      DROP TRIGGER user_login_handler ON auth.users;
    END IF;
  END $$;
  ...
  -- The ALTER statements have been incorporated into the original CREATE TABLE statements

  -- Create roster_entries table for game roster management
  ...
  -- Create indexes for the division tables
  ...
  ALTER TABLE league_divisions DISABLE ROW LEVEL SECURITY;
  ALTER TABLE tournament_divisions DISABLE ROW LEVEL SECURITY;
  GRANT ALL ON league_divisions TO authenticated;
  GRANT ALL ON tournament_divisions TO authenticated;

  -- Create function to ensure a league division exists before team membership is created
  CREATE OR REPLACE FUNCTION ensure_league_division()
  RETURNS TRIGGER AS $$
  BEGIN
    -- Skip if division is null
    ...
    IF NOT EXISTS (
      SELECT 1 FROM league_divisions 
      WHERE league_id = NEW.league_id AND name = NEW.division
    ) THEN
      -- Create the division if it doesn't exist
      INSERT INTO league_divisions (league_id, name, description)
      VALUES (NEW.league_id, NEW.division, 'Auto-created division');
    END IF;
  ...
  -- Create trigger to auto-create league divisions when needed
  CREATE TRIGGER ensure_league_division_trigger
  BEFORE INSERT OR UPDATE ON team_league_memberships
  FOR EACH ROW
  EXECUTE FUNCTION ensure_league_division();

  -- Create function to ensure a tournament division exists before team registration is created
  CREATE OR REPLACE FUNCTION ensure_tournament_division()
  RETURNS TRIGGER AS $$
  BEGIN
    -- Skip if division is null
    IF NEW.division IS NULL THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE tournament_id = NEW.tournament_id AND name = NEW.division
    ) THEN
      -- Create the division if it doesn't exist
      INSERT INTO tournament_divisions (tournament_id, name, description)
      VALUES (NEW.tournament_id, NEW.division, 'Auto-created division');
    END IF;
  ...
  -- Create trigger to auto-create tournament divisions when needed
  CREATE TRIGGER ensure_tournament_division_trigger
  BEFORE INSERT OR UPDATE ON tournament_teams
  FOR EACH ROW
  EXECUTE FUNCTION ensure_tournament_division();

  CREATE INDEX IF NOT EXISTS idx_roster_entries_game_id ON roster_entries(game_id);
  CREATE INDEX IF NOT EXISTS idx_roster_entries_player_id ON roster_entries(player_id);

  -- Create a view to get players and their information
  ...
  -- Create a stored procedure for attendance statistics
  CREATE OR REPLACE FUNCTION get_attendance_stats(time_range TEXT)
  RETURNS TABLE (
    player_id UUID,
    player_name TEXT,
    total_games BIGINT,
    games_attended BIGINT,
  ...
  BEGIN
    ...
    CASE time_range
      WHEN 'week' THEN
        start_date := current_date - interval '1 week';
      WHEN 'month' THEN
    ...
    WITH player_stats AS (
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN re.is_attending THEN g.id END) AS games_attended
      FROM
        players p
      CROSS JOIN games g
      LEFT JOIN roster_entries re ON g.id = re.game_id AND p.id = re.player_id
      WHERE
    ...
    SELECT
      ...
      ps.total_games,
      ps.games_attended,
      CASE WHEN ps.total_games > 0 THEN (ps.games_attended::NUMERIC / ps.total_games) ELSE 0 END AS attendance_rate
    FROM
      player_stats ps
    ORDER BY
      attendance_rate DESC, player_name;
  END;
  $$ LANGUAGE plpgsql;

  -- Grant permissions to the new table and function
  ALTER TABLE roster_entries DISABLE ROW LEVEL SECURITY;
  GRANT ALL ON roster_entries TO authenticated;
  GRANT EXECUTE ON FUNCTION get_attendance_stats(TEXT) TO authenticated;
